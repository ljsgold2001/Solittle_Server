...
router.post('/', async(req, res) => {
    if (!req.body.email || !req.body.password) {
        ...
        if(selectUserResult.length == 0){
            res.status(200).send(util.successFalse(statusCode.BAD_REQUEST, resMessage.CANNOT_FIND_USER));
        }else{
            ...
            if(selectUserResult[0].password == hashedEnterPw.toString('base64')){
                ...
                res.status(200).send(util.successFalse(statusCode.BAD_REQUEST, resMessage.NOT_CORRECT_PWD));
            }
        }
    }
});
